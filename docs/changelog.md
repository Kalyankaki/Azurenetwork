...
- bug: non-admin employers couldn't see *any* applications because `EmployerApplicants` and `EmployerDashboard` subscribed to the whole `applications` collection without a `where()` clause matching the Firestore read rule — the listener was rejected wholesale. Both pages now call `useApplications({ employerUid: user.uid })`. Also: `reassignInternshipEmployer` now backfills every application under a reassigned internship with the new rep's `employerUid / employerName / company`, logging `applications_reassigned` with the count — so historical apps follow the rep. Closes [#108](https://github.com/Kalyankaki/Azurenetwork/issues/108).
- bug: the EmployerDashboard candidate-matching tab was broken for non-admin employers because `useUsers()` subscribed to the whole `users` collection (admin-only read). `useUsers` now accepts `{ enabled }`; the dashboard subscribes only when an admin is viewing, and falls back to an "All Applicants" view (derived from the apps the employer already owns) for everyone else. Privacy-preserving — non-admin employers never read other users' docs. Closes [#110](https://github.com/Kalyankaki/Azurenetwork/issues/110).
- admin: AdminUsers profile modal gains an **Edit contact info** button — opens a small form to update `email / companyName / companyWebsite` on the user doc, with an opt-in checkbox to also backfill `contactEmail` (and `company` if changed) on every internship owned by that user. Logs `admin_user_contact_updated` with the diff. Note: this updates Firestore only; the Firebase Auth login email is unchanged. Closes [#112](https://github.com/Kalyankaki/Azurenetwork/issues/112).
- employer: bulk-download resumes from `/employer/applicants` — checkbox column in the "All Candidates" table, header select-all, and a `Download N resumes` action that zips the selected applicants' resumes via JSZip and saves as `<company>-<posting>-resumes-<date>.zip`. Cap of 50 per click; missing or unfetchable resumes are skipped with a count in the toast. Closes [#115](https://github.com/Kalyankaki/Azurenetwork/issues/115).
- employer: bulk-resume-download now also covers the **Top Candidates** tab of `/employer/applicants` and **both** candidate tabs (Top 5 + All Interns/Applicants) on `/employer`. Same banner + selection model lifted up so selections survive switching between tabs. Closes [#117](https://github.com/Kalyankaki/Azurenetwork/issues/117).
- bug: super-admin saw zero applications on `/employer/applicants` and `/employer` because PR #109 filtered the queries by `employerUid: user.uid` — and super-admins typically don't own any postings. The filter is now skipped for admins (their `isAdmin()` branch in the rule allows the unfiltered listener), so super-admins see every application + posting on the employer pages. Non-admin employers still get the scoped queries unchanged. Closes [#119](https://github.com/Kalyankaki/Azurenetwork/issues/119).
- admin: new **"Interns sitting on an offer > 7 days"** card on `/admin`. Applications now stamp `offeredAt: serverTimestamp()` when their status flips to `offered`, and the dashboard ages each open offer off that timestamp (falling back to `updatedAt` for legacy rows). Per-row links to the applicant profile + internship modal; rows over 14 days highlighted red. Closes [#121](https://github.com/Kalyankaki/Azurenetwork/issues/121).
...
- admin: `/admin/applications` adds an **All Companies** dropdown next to the Status filter. Options are derived live from the application set; composes with search, status, sort, and stale filters. Closes [#104](https://github.com/Kalyankaki/Azurenetwork/issues/104).
- admin: when a specific company is selected on `/admin/applications`, the status tiles (Pending / Under Review / Shortlisted / Offered / Offer Accepted / etc.) and the Stale count now reflect that company only, not the whole platform. Closes [#106](https://github.com/Kalyankaki/Azurenetwork/issues/106).
