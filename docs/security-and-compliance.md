...
| **R3** ✅ | **Privacy Notice and Terms of Use published** at `/privacy` and `/terms`, linked from the homepage footer; **cookie/storage notice banner** appears on first visit and is dismissible. | — | — | `src/pages/PrivacyPolicy.jsx`, `src/pages/Terms.jsx`, `src/components/CookieNotice.jsx` |
...
| **R6** ✅ | **Resume virus scanning** via Firebase Cloud Function (`functions/scanResume.js`) on every Storage upload. Uses VirusTotal v3; flagged files are deleted from Storage and the user is shown a quarantine notice on their profile. **Action required:** `firebase deploy --only functions:scanResume` and set `VIRUSTOTAL_KEY` secret. | — | — | `functions/scanResume.js` |
...
| **R9** ✅ | **Activity log TTL.** Each entry stamps `expiresAt = createdAt + 12 months`; Firestore TTL deletes them automatically. **Action required:** enable TTL policy on `activity.expiresAt` in Firebase Console. | — | — | `src/services/firestore.js` `logActivity` |
...
| **R11** ✅ | **Data-export and self-service deletion** added to `/intern/profile` → "Privacy" section. Export is a JSON download of user doc + applications + messages. Delete removes user doc, applications, messages, resumes, and the Firebase Auth account (with re-authentication fallback). Firestore + Storage rules updated to permit owner self-delete. | — | — | `src/pages/intern/InternProfile.jsx`, `src/services/firestore.js` (`exportUserData`, `selfDeleteUser`) |
| **R12** 🟨 | **PITR / scheduled backups** documented in `docs/operations.md`. **Action required:** toggle on in Firebase Console (no code path). | Low | High | Firebase project config |
...
| **R14** ✅ | **Dependabot config** at `.github/dependabot.yml`: weekly npm updates for app + functions, monthly GitHub Actions, grouped by Firebase / React / build. **Action required:** enable Dependabot alerts in repo settings. | — | — | `.github/dependabot.yml` |
| **R15** ✅ | **Real-time alerting on sensitive admin actions.** `logActivity()` for `roles_changed`, `user_deleted`, `employer_approved`, `internship_approved/rejected/deleted` posts to `/api/admin-alert`, which emails the admin allowlist via Resend. Falls back to a Firestore notifications doc on failure. | — | — | `src/services/firestore.js`, `api/admin-alert.js` |
| **R16** ✅ | **Notifications collection TTL.** Each notification stamps `expiresAt = createdAt + 6 months`. **Action required:** enable TTL policy on `notifications.expiresAt` in Firebase Console. | — | — | `src/services/firestore.js` `sendAdminNotification` |
